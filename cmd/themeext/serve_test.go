...
// TestServe_RunE_RequiresThemeID covers newCmdServe's --theme-id gate. A
// non-interactive caller gets the structured missing-flag error; the gate lives
// in RunE (not PreRunE) because the human picker needs the validated token.
func TestServe_RunE_RequiresThemeID(t *testing.T) {
	cmd := newCmdServe(&cmdutil.Factory{})
	_ = cmd.Flags().Set("path", t.TempDir())
	err := cmd.RunE(cmd, nil)
	if err == nil || !strings.Contains(err.Error(), "--theme-id") {
		t.Errorf("expected missing --theme-id error, got %v", err)
	}
}
