...
### Added
- `themes +page` block rows now carry `cname`, the block's display name, taken from the `schemas` map the response already returns (`presets[0].cname`) — no extra request. A generated block's type is a random `blocks/gen_…`, so re-editing one had no way to match it against what a merchant asked for.
...
- `cli-user-id` request header on API calls, for audit attribution. Commands that go through the auth gate now send the id of the logged-in user alongside the store token, so a write can be traced back to a person. The value comes from the session recorded at login, or from `SHOPLAZZA_CLI_USER_ID` — set that when `SHOPLAZZA_ACCESS_TOKEN` injects a token, since an injected token carries no login state. An unresolvable id just omits the header; no command fails over it.
...
- Errors now follow the same audience split as data output. At a `pretty`/`table` terminal a failure prints a readable `Error:` line — with the hint, the API error code and HTTP status (`Code: InvalidParameter (HTTP 400)`), and the failing endpoint / request id when the server returned them — instead of a raw JSON blob. Piped, in CI, with `--no-input`, or in any `json`/`ndjson`/`csv` mode, stderr still carries the `{"ok":false,"error":{…}}` envelope, so agents and scripts keep parsing errors as JSON. Both conditions must hold for the human line (a human-oriented format **and** a terminal on stderr), so redirecting stderr always yields the JSON envelope.
